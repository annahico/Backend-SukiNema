...
function Checkout(props: Props) {
  async function handleCheckout(): Promise<void> {
    ...
    const reqBody = {
      user: 1,
      cart: props.cart.map((e) => ({
        showingId: e.ticket.showingId,
        seat: e.ticket.seat,
      })),
    };
    ...
    if (data.err) {
      alert(
        `Your ticket "${data.seat}" has been recently reserved by another guest.`
      );
    ...
    <div className="checkout">
      ...
      <div className="checkout-form">
        ...
        <p className="checkout--warning">
          <span>Note:</span> Payment section is for appearance only, no payment
