      ...
      return (
        <div className="cart-item" key={e.ticket.showingId + e.ticket.seat}>
          <div className="cart-item-poster">
            <img
              src={e.showing.Movie.poster}
              alt={e.showing.Movie.title + " poster"}
            />
          </div>
          <div className="cart-item-details">
            <h2 className="cart-item--title">{e.showing.Movie.title}</h2>
            <p className="cart-item--text">
              <span>Time:</span> {e.showing.time}
            </p>
            <p className="cart-item--text">
              <span>Room:</span> {e.showing.room}
            </p>
            <p className="cart-item--text">
              <span>Seat:</span> {e.ticket.seat}
            </p>
          </div>
          <div
            className="cart-remove"
            onClick={() =>
              props.removeCartItem(e.ticket.showingId, e.ticket.seat)
            }
          >
            <p>Delete</p>
            <Trash />
